...
6. **allDay**（是否全天事件）  
7. **url**（活动链接，通常为网页或海报）  
8. **calendar**（活动所属日历，从提供的日历名称列表中选择最合适的，并填充在此字段中）  
9. **status**（活动状态，如是否已发布）  
10. **eventIdentifier**（活动唯一标识符，请为每个事件生成一个 UUID）  
11. **remarks**（如有不确定的信息，请在此注明）
...
11. **异常时间处理**：对于每个事件：  
    ...
    - 如果未定义开始时间和结束时间，则视为全天事件，allDay设置为true， 字段： `allDay`。  
12. 如果文中未明确指定活动年份，请使用【用户上下文】中配置的年份，字段：`currentYear`  
13. **总结文本内容**：为每个事件生成500个字左右的摘要，填充到 `notes` 字段中。
14. 若从文本内容中，未识别到任何地理位置信息，则自动从文本中提取线上活动标识：【抖音直播、视频号直播、腾讯会议、钉钉会议、飞书会议、Zoom会议、线上会议】，并将 location 字段设为对应标识。若同时存在多个线上标识，按优先级取首个匹配项；如果未匹配任何线上标识，则取默认值：线上活动。
15. 主题很关键，需要明确传递活动或事件的主题，如果文中已经明确提到了活动的主题，就使用已经定义的主题。如果未明确定义，总结文本摘要作为主题，字数控制在20字以内，最终填充到 `title` 字段。
