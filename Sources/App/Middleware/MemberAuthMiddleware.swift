  ...
  func respond(to request: Request, chainingTo next: AsyncResponder) async throws -> Response {
    ...
    guard member.isActive else {
      request.logger.notice(
        "Inactive member attempted access: \(member.id?.uuidString ?? "unknown")")
    ...
    if let endDate = member.membershipEndDate {
      if endDate < Date() {
        request.logger.notice(
          "Expired membership attempted access: \(member.id?.uuidString ?? "unknown")")
    ...
    // 6. 记录成功的认证
    request.logger.info(
      "Successful authentication for member: \(member.id?.uuidString ?? "unknown")")
...
extension Request {
  /// 获取当前认证的会员
  fileprivate var authenticatedMember: Member? {
